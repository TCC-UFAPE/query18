...
# Construir máscara para chromium: procurar 'chromium' em CVE ou Arquivos Analisados quando existirem
if cve_col is not None:
    mask_cve = all_df[cve_col].astype(str).str.contains('chromium', case=False, na=False)
else:
    mask_cve = pd.Series([False]*len(all_df))

if files_col is not None:
    mask_files = all_df[files_col].astype(str).str.contains('chromium', case=False, na=False)
else:
...
if httpd_df.empty:
    print('Nenhum CVE relacionado a "chromium" encontrado nas abas. Vou usar todos os dados para gerar um gráfico geral.')
...
plt.title('Comparativo de Detecções por LLM (chromium)')
