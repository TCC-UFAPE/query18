...
def run_task_1_and_2(vulnerabilities):
    ...
    print("   [FILTRO ATIVO] Processando apenas projeto chromium")
...
if __name__ == "__main__":
    ...
    if all_vulnerabilities and all_tags_map:
        # Filtrar apenas vulnerabilidades do chromium
        httpd_vulns = [v for v in all_vulnerabilities if v.get('project_name', '').lower() == 'chromium']
        print(f"\n[FILTRO] {len(httpd_vulns)} vulnerabilidades do chromium de {len(all_vulnerabilities)} totais")
        
        if not httpd_vulns:
            print("\n[ERRO] Nenhuma vulnerabilidade do chromium encontrada.")
        else:
            # Executar tarefas da query7 apenas com chromium
            run_task_1_and_2(httpd_vulns)
            time.sleep(1)
            run_task_3_and_4(httpd_vulns, all_tags_map)
            time.sleep(1)
            
            # Executar análise combinada (query7 + query10) apenas com chromium
